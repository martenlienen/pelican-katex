...
import pelican_katex.rendering as settings
...
class RenderServer:
    ...
    def timeout_error(self, timeout):
        ...
        return settings.KaTeXError(message)

    @staticmethod
    def build_command(socket=None, port=None):
        cmd = [settings.KATEX_NODEJS_BINARY, SCRIPT_PATH]
        ...
        if settings.KATEX_PATH:
            cmd.extend(["--katex", str(settings.KATEX_PATH)])
    ...
    def start(cls):
        ...
        if os.name == "posix":
            process, sock = cls.start_unix_socket(rundir, settings.KATEX_STARTUP_TIMEOUT)
        else:
            # Non-unix systems (i.e. Windows) do not necessarily support unix
            # domain sockets for IPC, so we fallback to network sockets.
            process, sock = cls.start_network_socket(rundir, settings.KATEX_STARTUP_TIMEOUT)
...
def render_latex(latex, options=None):
    ...
    try:
        response = server.render(request, settings.KATEX_RENDER_TIMEOUT)

        if "html" in response:
            return response["html"]
        elif "error" in response:
            raise settings.KaTeXError(response["error"])
        else:
            raise settings.KaTeXError("Unknown response from KaTeX renderer")
    except socket.timeout:
        raise settings.KaTeXError(TIMEOUT_EXPIRED_TEMPLATE.format(latex))


def render_latex_katex(latex, options=None):
    """Ask the KaTeX server to render some LaTeX.
